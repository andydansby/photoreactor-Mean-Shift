...
public:
	...
	{
		return "2d Mean Shift produces a Edge Preserving smooth which replaces each pixel with the mean of the pixels in a range neighborhood and whose value is within a range distance.";
	}
	...
	int GetUIParameters (UIParameters* pParameters)
	{
		...
		AddParameter( PARAM_RADIUS ,"Radius", 5.0, 1.0, 20.0, TYPE_SLIDER, 0.0);//default  min   max
		AddParameter( PARAM_COLORDIST ,"Color Distance", 25.0, 1.0, 128.0, TYPE_SLIDER, 0.0);//default  min   max
	...
	virtual void Process_Data (BYTE* pBGRA_out,BYTE* pBGRA_in, int nWidth, int nHeight, UIParameters* pParameters)
	{
		...
		double nColorDistance = GetValue(PARAM_COLORDIST);

		int radius = (int)dRadius;//convert radius to an integer
		int colorDistance = (int) nColorDistance;

		int size = (radius + radius) + 1;// we want an odd sized kernel
		...
		for(int y = 0; y < nHeight; y++)
		{
			for(int x = 0; x < nWidth; x++)
			{
				...
				//accumulate colors
				for(int i = max(0, x - size); i <= MIN(nWidth - 1, x + size); i++)//for(int j = max(0, y - size); j <= MIN(nHeight - 1, y + size); j++)
				{
					for(int j = max(0, y - size); j <= MIN(nHeight - 1, y + size); j++)//for(int i = max(0, x - size); i <= MIN(nWidth - 1, x + size); i++)
					{
						...
						int redKernelCenter   = pBGRA_in[(x + y * nWidth) * 4 + CHANNEL_R];
						int greenKernelCenter = pBGRA_in[(x + y * nWidth) * 4 + CHANNEL_G];
						int blueKernelCenter  = pBGRA_in[(x + y * nWidth) * 4 + CHANNEL_B];
						// this stays in center of kernel

						int redDiff   =  abs(redKernelSlider - redKernelCenter);
						int greenDiff =  abs(greenKernelSlider - greenKernelCenter);
						int blueDiff  =  abs(blueKernelSlider - blueKernelCenter);


						if (redDiff <= colorDistance)
						{
							Rsum = Rsum + redKernelSlider;
							Rdivisor ++;
						}

						if (greenDiff <= colorDistance)
						{
							Gsum = Gsum + greenKernelSlider;
							Gdivisor ++;							
						}

						if (blueDiff <= colorDistance)
						{
							Bsum = Bsum + blueKernelSlider;
							Bdivisor ++;
						}
				...
				{//output
					pBGRA_out[(x + y * nWidth) * 4 + CHANNEL_R] = CLAMP255(Ravg);
					pBGRA_out[(x + y * nWidth) * 4 + CHANNEL_G] = CLAMP255(Gavg);
					pBGRA_out[(x + y * nWidth) * 4 + CHANNEL_B] = CLAMP255(Bavg);
				}//end output
...
extern "C" __declspec(dllexport) char* GetPluginID()
{
	...
	return "com.lumafilters.MeanShift";
